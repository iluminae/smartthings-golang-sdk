...
/*
CreateDeviceProfile Create a device profile
...
func (a *DeviceprofilesApiService) CreateDeviceProfile(ctx _context.Context, authorization string, localVarOptionals *CreateDeviceProfileOpts) (DeviceProfile, *_nethttp.Response, error) {
	...
	localVarPath := a.client.cfg.BasePath + "/deviceprofiles"
	...
	if localVarOptionals != nil && localVarOptionals.Request.IsSet() {
		localVarOptionalRequest, localVarOptionalRequestok := localVarOptionals.Request.Value().(CreateDeviceProfileRequest)
		if !localVarOptionalRequestok {
			return localVarReturnValue, nil, reportError("request should be CreateDeviceProfileRequest")
		}
...
/*
DeleteDeviceProfile Delete a device profile
...
func (a *DeviceprofilesApiService) DeleteDeviceProfile(ctx _context.Context, authorization string, deviceProfileId string) (map[string]interface{}, *_nethttp.Response, error) {
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/deviceprofiles/{deviceProfileId}"
	localVarPath = strings.Replace(localVarPath, "{"+"deviceProfileId"+"}", _neturl.QueryEscape(parameterToString(deviceProfileId, "")) , -1)
...
/*
GetDeviceProfile GET a device profile
...
/*
ListDeviceProfiles List all device profiles for the authenticated user
...
func (a *DeviceprofilesApiService) ListDeviceProfiles(ctx _context.Context, authorization string, localVarOptionals *ListDeviceProfilesOpts) (PagedDeviceProfiles, *_nethttp.Response, error) {
	var (
		...
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  PagedDeviceProfiles
	)
	...
	if localVarOptionals != nil && localVarOptionals.ProfileId.IsSet() {
		t:=localVarOptionals.ProfileId.Value()
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("profileId", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("profileId", parameterToString(t, "multi"))
		}
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
...
/*
UpdateDeviceProfile Update a device profile.
...
func (a *DeviceprofilesApiService) UpdateDeviceProfile(ctx _context.Context, authorization string, deviceProfileId string, localVarOptionals *UpdateDeviceProfileOpts) (DeviceProfile, *_nethttp.Response, error) {
	...
	// body params
	if localVarOptionals != nil && localVarOptionals.Request.IsSet() {
		localVarOptionalRequest, localVarOptionalRequestok := localVarOptionals.Request.Value().(UpdateDeviceProfileRequest)
		if !localVarOptionalRequestok {
			return localVarReturnValue, nil, reportError("request should be UpdateDeviceProfileRequest")
		}
		localVarPostBody = &localVarOptionalRequest
	}
